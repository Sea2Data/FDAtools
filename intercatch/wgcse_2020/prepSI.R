...
annotate_logbooks <- function(stock, logb){
  ...
  logb$MASKEVIDDE <- as.character(logb$MASKEVIDDE)
  # annotate metier on logbooks
  logb[is.na(logb$MASKEVIDDE), "MASKEVIDDE"] <- "NON"
  
  logb <- merge(logb, metier, all.x=T, by=names(logb)[names(logb) %in% names(metier)])
  if (nrow(logb)!=nr){
...
write_intercatch <-function(species, data){
  ...
  stream <- file(paste(output, paste(species, Year, "nor_intercatch_wgcse.csv", sep="_"), sep="/"), open="w")
